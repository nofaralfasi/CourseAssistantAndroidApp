...
public class Course
{
    ...
    public Course(String courseName, float points, Calendar startDate, Calendar endDate, int courseColor, List<CourseDay> courseDays)
    {
        ...
        this.studyGroups=new StudyGroup[5];
    }
    ...
    {
        return Arrays.asList(studyGroups);
    }
    ...
    public StudyGroup[] GetStudyGroupsAsArray()
    {
    ...
    {
        return exams[term.ordinal()] == null;

    }

    public boolean CheckStudyGroupValidate(Term term)
    {
        return studyGroups[term.ordinal()] == null;
    ...
    public boolean CheckStudyGroupDate(Calendar calendar, int skipIndex)
    {

        for (int i = 0; i < studyGroups.length; i++)
        {
            if (studyGroups[i] != null && i != skipIndex)
            {
                if (calendar.get(Calendar.YEAR) == studyGroups[i].GetStudyGroupDateAsCalendar().get(Calendar.YEAR)
                        && calendar.get(Calendar.DAY_OF_MONTH) == studyGroups[i].GetStudyGroupDateAsCalendar().get(Calendar.DAY_OF_MONTH)
                        && calendar.get(Calendar.MONTH) == studyGroups[i].GetStudyGroupDateAsCalendar().get(Calendar.MONTH))
                {
                    return false;
                }
